Configuration WvdWin10Config
{
    ...
    Node localhost {
        ...
        Script FsLogixProfileGroupExclusion {
            DependsOn = '[xPackage]FsLogix'
            GetScript = { return @{'Result' = ''} }
            SetScript = { Add-LocalGroupMember -Group "FSLogix Profile Exclude List" -Member ("VA\VAOITWindowsVirtualDesktop") -ErrorAction SilentlyContinue }
            TestScript = {
                $Members = Get-LocalGroupMember -Group "FSLogix Profile Exclude List" | Foreach-Object {$_.Name}
                If ($Members -contains "VA\VAOITWindowsVirtualDesktop") { Return $true }
                Else { Return $false }
            }
        }
        Script FsLogixOfficeGroupExclusion {
            DependsOn = '[xPackage]FsLogix'
            GetScript = { return @{'Result' = ''} }
            SetScript = { Add-LocalGroupMember -Group "FSLogix ODFC Exclude List" -Member ("VA\VAOITWindowsVirtualDesktop") -ErrorAction SilentlyContinue }
            TestScript = {
                $Members = Get-LocalGroupMember -Group "FSLogix ODFC Exclude List" | Foreach-Object {$_.Name}
                If ($Members -contains "VA\VAOITWindowsVirtualDesktop") { Return $true }
                Else { Return $false }
            }
        }
